    ...
    while(!q.empty()) {
        ...
        if (tmp.type == "in") {
            ...
            for (int child : tmp.children) {
                if (graph[child].type == "out" || graph[child].worklist == -1 || graph[child].type == "reg") {
                    ...
                    if (worknum.size() == 0) {
                        worknum.insert(graph[child].worklist);
                    }
                    else if (worknum.count(child) == 0) {
                        if (is_blacked(tmp.gate_parser, blacked)) {
                            tmp.worklist = -1;
                        }
                        else {
            ...
            if (flag == 0) 
                tmp.worklist = graph[*worknum.begin()].worklist;
        }
        if (tmp.parents[0] != -1) {
...
void encoding_circuit(string benchpath, context &c, expr_vector &outlist, expr_vector &inlist, map<string, int> &inlistDict, string v_name = "i") {
    ifstream bench(benchpath);
    string templine;
...
void encoding_fault(string benchpath, set<string> blacked, set<string> outs, bool isOpt, context &c, solver &s, int nof, string fault_type, string countermeasures,
         int variate, string outpath_cnf, expr_vector &inlist, map<string, int> &inlistDict, expr_vector &outlistP, vector <expr_vector> &conditions, int opt_meth) {
    set<string> binaryOp = {"xnor", "xor", "and", "nand", "nor", "or"};
    set<string> unaryOp = {"reg", "not"};
    map<string, int> gate_type;
    ifstream bench(benchpath);
...
void encoding_fault_withoutstage(string benchpath, set<string> blacked, set<string> outs, bool isOpt, context &c, solver &s, int nof, string fault_type, string countermeasures,
         int variate, string outpath_cnf, expr_vector &inlist, map<string, int> &inlistDict, expr_vector &outlistP, expr_vector &conditions, int opt_meth, int stage_ind) {
    set<string> binaryOp = {"xnor", "xor", "and", "nand", "nor", "or"};
    set<string> unaryOp = {"reg", "not"};
    map<string, int> gate_type;
    ifstream bench(benchpath);
...
void verify_correctness(string pure_circuit, string circuit_with_countermeasure, context &c, solver &s, string outpath_cnf, string countermeasures, string solver_select) {
    // cout << "test 1" << endl;
    ...
    encoding_circuit(pure_circuit, c, outlist1, inlist, inlistDict);
    encoding_circuit(circuit_with_countermeasure, c, outlist2, inlist, inlistDict);
...
void verify_consistency(string circuit_with_countermeasure, context &c, solver &s, string outpath, string solver_select) {
    expr_vector outlist1(c);
    expr_vector outlist2(c);
    expr_vector inlist(c);
    map<string, int> inlistDict;
    // cout << inlist.size() << endl;
    encoding_circuit(circuit_with_countermeasure, c, outlist1, inlist, inlistDict, "c1");
    encoding_circuit(circuit_with_countermeasure, c, outlist2, inlist, inlistDict, "c2");
    // cout << outlist1.size() << endl;
    // cout << outlist2.size() << endl;
...
void verify_inject(string benchpath1, string benchpath2, set<string> blacked, set<string> outs, bool isOpt, context &c, solver &s, int nof,
            string fault_type, string countermeasures, int variate, string outpath_cnf, int opt_meth, bool encode_stage, string solver_select) {
    set<string> binaryOp = {"xnor", "xor", "and", "nand", "nor", "or"};
    set<string> unaryOp = {"reg", "not"};
    expr_vector outlist1(c);
    expr_vector outlist2(c);
    ...
    encoding_circuit(benchpath1, c, outlist1, inlist, inlistDict);
    ...
    if (encode_stage == false) {
        for (int stage_ind = 0; stage_ind <= max_stage; stage_ind++) {
            ...
            encoding_fault_withoutstage(benchpath2, blacked, outs, isOpt, c, s, nof, fault_type, countermeasures, variate, outpath_cnf, inlist, inlistDict, outlist3, condition, opt_meth, stage_ind);
        ...
        encoding_fault(benchpath2, blacked, outs, isOpt, c, s, nof, fault_type, countermeasures, variate, outpath_cnf, inlist, inlistDict, outlist2, conditions, opt_meth);
        ...
        if (solver_select == "SAT") {
            ...
            if (max_stage > 0) {
                int stage_coeffs[max_stage + 1];
                fill(stage_coeffs, stage_coeffs + max_stage + 1, 1);
...
int main(int argc, char **argv) {
    ...
    if (task == 1) {
        ...
        verify_correctness(circuit, circuit_countermeasures, c, s, outpath, countermeasure, solver_select);
    ...
    if (task == 2) {
        ...
        if (countermeasure == "detection") {
            verify_consistency(benchpath, c, s, outpath, solver_select);
        }
        else {
    ...
    if (task == 3) {
        ...
        // cout << nof << endl;
        int variate = stoi(argv[6]); // number of stages
        string countermeasures(argv[7]);
        string fault_type(argv[8]);
        string str7(argv[9]);
        bool isOpt;
        if (str7 == "opt") 
            isOpt = true;
        ...
        int opt_meth = stoi(argv[10]);
        bool encode_stage;
        string str12(argv[11]);
        // cout << argv[11] << endl;
        if (str12 == "true") {
            encode_stage = true;
        }
        ...
        verify_inject(benchpath, benchpath, blacked, outs, isOpt, c, s, nof, fault_type, countermeasures, variate, outpath_cnf, opt_meth, encode_stage, solver_select);
    ...
    if (task == 4) {
        ...
        verify_inject(benchpath_pure, benchpath, blacked, outs, isOpt, c, s, nof, fault_type, countermeasures, variate, outpath_cnf, opt_meth, encode_stage, solver_select);
